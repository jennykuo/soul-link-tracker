...
var selectedGame = games[localStorage.getItem('selectedGame')] ? localStorage.getItem('selectedGame') : 'custom';
			...
			'<table class="ui table sortable selectable">' +
				'<thead>' +
					'<tr>' +
						...
						'<th class="center aligned four wide">Encounter 1</th>' +
						'<th class="center aligned four wide">Nickname 1</th>' +
						'<th class="center aligned four wide">Encounter 2</th>' +
						'<th class="center aligned four wide">Nickname 2</th>' +
		...
		string += '<tr' + (location.order !== undefined ? ' class="customLocation"' : '' ) + '>' +
			...
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-nickname')) + '">' +
				'<div class="ui' + (darkTheme ? ' inverted' : '') + ' fluid input">' +
					'<input autocomplete="off" maxlength="' + game.nameLimit + '" class="nickname-input" value="' + (localStorage.getItem(game.id + location.value + '-nickname') ? escapeHTML(localStorage.getItem(game.id + location.value + '-nickname')) : '') + '" id="' + game.id + locationValue + '-nickname" name="nickname" placeholder="Nickname123" type="text">' +
				'</div>' +
			'</td>' +
			'<td data-sort-value="' + index + '">' + '<div title="Delete" class="ui' + (darkTheme ? ' inverted' : '') + ' basic show-sm float-right singleReset icon button" data-location-id="' + locationValue + '"><i class="remove icon"></i></div>' + escapeHTML(location.name) + '</td>' +
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-encounter2')) + '">' +
				...
				'</div>' +
			'</td>' +
			'<td data-sort-value="' + escapeHTML(localStorage.getItem(game.id + location.value + '-nickname2')) + '">' +
				'<div class="ui' + (darkTheme ? ' inverted' : '') + ' fluid input">' +
					'<input autocomplete="off" maxlength="' + game.nameLimit + '" class="nickname2-input" value="' + (localStorage.getItem(game.id + location.value + '-nickname2') ? escapeHTML(localStorage.getItem(game.id + location.value + '-nickname2')) : '') + '" id="' + game.id + locationValue + '-nickname2" name="nickname2" placeholder="Nickname2" type="text">' +
				'</div>' +
			'</td>' +
...
function populateLocation(game, data) {
	...
	var encounterElm = $('#' + id + '-encounter2');
	var nicknameElm = $('#' + id + '-nickname2');
	...
	if (data.encounter2 !== null && data.encounter2 !== '') {
		encounter2Elm.dropdown('set value', data.encounter2);
		encounter2Elm.dropdown('set text', '<img class="pkmn" src="img/' + data.encounter2 + '">' + data.name);
		encounter2Elm.data('name', data.name);
		localStorage.setItem(id + '-encounter2', data.encounter2);
		localStorage.setItem(id + '-name', data.name);
	} else {
		encounter2Elm.closest('td').data('sortValue', '');
		encounter2Elm.dropdown('clear');
		localStorage.removeItem(id + '-encounter2');
		localStorage.removeItem(id + '-name');
	}

	if (data.nickname2 !== null && data.nickname2 !== '') {
		nickname2Elm.val(data.nickname2);
		localStorage.setItem(id + '-nickname2', data.nickname2);
	} else {
		nickname2Elm.val('').closest('td').data('sortValue', '');
		localStorage.removeItem(id + '-nickname2');
	}
